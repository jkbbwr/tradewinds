...
from tradewinds_api_client import AuthenticatedClient, Client
...
from tradewinds_api_client.models import LoginRequest, RegisterRequest
...
def main():
    ...
    with client as c:
        if should_register:
            ...
            register_request = RegisterRequest(email=email, name=name, password=password)

            register_response = execute_with_rate_limit(
                tradewinds_web_auth_controller_register.sync_detailed, client=c, body=register_request
            )
            ...
            if hasattr(parsed_register, "data"):
                ...
                print(
                    f"Registration returned: status_code={getattr(register_response, 'status_code', 'Unknown')}, content={getattr(register_response, 'content', 'Unknown')}"
                )
                return
        else:
        ...
        login_response = execute_with_rate_limit(
            tradewinds_web_auth_controller_login.sync_detailed, client=c, body=login_request
        )
        ...
        if hasattr(parsed_login, "data") and hasattr(parsed_login.data, "token"):
            ...
            print(
                f"Login failed or unexpected response: status_code={getattr(login_response, 'status_code', 'Unknown')}, content={getattr(login_response, 'content', 'Unknown')}"
            )
    ...
    if token:
        ...
        with auth_client as ac:
            revoke_response = execute_with_rate_limit(tradewinds_web_auth_controller_revoke.sync_detailed, client=ac)
            if hasattr(revoke_response, "status_code") and revoke_response.status_code == 204:
                print("Token successfully revoked.")
            else:
                print(
                    f"Token revocation failed: status_code={getattr(revoke_response, 'status_code', 'Unknown')}, content={getattr(revoke_response, 'content', 'Unknown')}"
                )
