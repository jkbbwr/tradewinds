defmodule TradewindsWeb.MarketControllerTest do
  ...
  describe "POST /api/v1/market/orders" do
    test "creates an order successfully", %{conn: conn, port: port, good: good} do
      conn = post(conn, ~p"/api/v1/market/orders", %{
        port_id: port.id,
        good_id: good.id,
        side: "buy",
        price: 100,
      ...
      assert %{"id" => id, "side" => "buy", "price" => 100, "total" => 50, "status" => "open"} = json_response(conn, 201)["data"]
      assert id
    end
  end
  ...
  describe "GET /api/v1/market/orders" do
    test "lists orders", %{conn: conn, company: company, port: port, good: good} do
      order = Factory.insert(:order, company: company, port: port, good: good, side: :sell, price: 50, total: 100, remaining: 100)
      
      conn = get(conn, ~p"/api/v1/market/orders", port_id: port.id, good_id: good.id, side: "sell")
  ...
  describe "GET /api/v1/market/blended-price" do
    test "calculates blended price", %{conn: conn, company: company, port: port, good: good} do
      Factory.insert(:order, company: company, port: port, good: good, side: :sell, price: 50, total: 100, remaining: 100)
      Factory.insert(:order, company: company, port: port, good: good, side: :sell, price: 60, total: 100, remaining: 100)
      
      conn = get(conn, ~p"/api/v1/market/blended-price", port_id: port.id, good_id: good.id, side: "sell", quantity: 150)
  ...
  describe "POST /api/v1/market/orders/:id/fill" do
    test "fills an order", %{conn: conn, company: company, port: port, good: good} do
      ...
      {:ok, _} = Tradewinds.Companies.record_transaction(seller_company.id, 1_000_000, :market_trade, :market, Ecto.UUID.generate(), DateTime.utc_now())
      ...
      {:ok, order} = Tradewinds.Market.post_order(
